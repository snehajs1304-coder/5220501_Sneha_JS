...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    int* distinct_ranked = (int*)malloc(ranked_count * sizeof(int));
    int distinct_ranked_count = 0;
    
    if(ranked_count> 0){
        distinct_ranked[distinct_ranked_count++] = ranked[0];
        for(int i=1;i<ranked_count;i++){
            if(ranked[i] != ranked[i - 1]){
                distinct_ranked[distinct_ranked_count++] = ranked[i];
            }
        }
    }
    ...
    *result_count = player_count;
    ...
    int rank_index = distinct_ranked_count - 1;
    
    for(int i=0;i<player_count;i++){
        while(rank_index >= 0 && player[i] >= distinct_ranked[rank_index]){
            rank_index--;
        }
